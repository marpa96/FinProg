...
This is the main ingestion path for Rocket Money now. It writes an optional raw JSON snapshot and upserts the source into one SQLite database with sync runs, page metadata, raw payload snapshots, normalized transaction/category/account/service/subscription tables, and per-transaction detail/history tables for split, related, rule, and chart data. The default update is quick: `--transaction-mode quick` stops paging when it reaches an already-known transaction, and `--detail-mode quick` fetches only missing detail/history bundles. Use `--transaction-mode full --detail-mode full` for a deep reconciliation pass that refetches old detail bundles and reports existing transactions whose Rocket Money detail data changed. Deep detail mode writes each fetched bundle immediately, waits between bundles by default, supports `--detail-limit` and `--detail-recent-days` budgets, and pauses before resuming if Rocket Money returns 403/429/temporary server errors. Use `--detail-mode skip` when you only need the transaction list.
...
`main.py` installs Python dependencies from `requirements.txt`, installs the Playwright browser when needed, opens the browser login helper, then syncs Rocket Money into `data/private/rocketmoney.db` and writes a snapshot to `data/private/rocketmoney_transactions.json`. The default update uses quick transaction paging and quick detail enrichment; it prints how many new transactions it found. Pass `--rocketmoney-transaction-mode full --rocketmoney-detail-mode full` for a deep reconciliation pass or `--rocketmoney-detail-mode skip` for transactions only. Deep-mode pacing can be tuned with `--rocketmoney-detail-request-delay` and `--rocketmoney-detail-throttle-delay`. The Python API also maintains source-agnostic sync schedule state in `data/private/sync_scheduler.db`: quick sync is foreground/user-facing, daily deep scan checks recent history, weekly and monthly lanes reconcile older history with conservative budgets, and only one Rocket Money lane runs at a time. The browser helper uses a persistent profile under `data/private/`, so after the first successful login it can usually refresh cookies without pasting new cURLs. If Rocket Money shows MFA, CAPTCHA, or a risk challenge, complete it in the opened browser.
