...
"""
    ...
    if missing_markers:
        ...
        failures.append(
            {
                "ledger_id": "WEB-002",
                "expected": "The React app exposes the simple home page, dedicated buckets page, and planner view.",
    ...
    if "This balance uses the current forecast day" not in app_source or "Today Variable Expense" not in app_source:
        source_artifact = write_artifact(SCRIPT_NAME, "current_day_source_snapshot.txt", app_source)
        failures.append(
            {
                "ledger_id": "WEB-004",
                "expected": "The app labels the daily net as Day Net, uses the current forecast day for the current balance view, and supports adding a variable expense for today.",
                "observed": "The app source is missing the expected current-day balance or today's-variable-expense markers.",
                "artifact": display_path(source_artifact),
            }
        )
